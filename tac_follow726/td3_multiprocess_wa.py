...
def worker(id, td3_trainer, rewards_queue, replay_buffer, max_episodes, max_steps, batch_size, explore_steps, \
    ...
    env = UnityEnv(env_name, worker_id=id+3, use_visual=False, use_both=True)
    action_range=8.
    state_dim = 6
    # state_dim = 279
    action_dim = 2
...
def state_process(s, s0):
    ...
    dis=np.abs(relative_x-(x0-object_x0))+np.abs(relative_z-(z0-object_z0))
    coord_list=np.transpose([x,z])
    # coord_list=np.transpose([relative_x,relative_z])
    weighted_average = np.dot(dis[np.newaxis,:], coord_list)/(np.sum(dis)+1e-5)
    # print('weighted average: ',weighted_average)

    processed_state=np.concatenate((s[:6], weighted_average[0])) # reduce 1 dim in weighted_average
...
if __name__ == '__main__':
    ...
    model_path = './model/td3_weighted_average'
    ...
    if args.train:
        ...
        while True:  # keep geting the episode reward from the queue
            ...
            if len(rewards)%50==0 and len(rewards)>0:
                plot(rewards)
                np.savetxt('rewards.txt', np.array(rewards)[:, np.newaxis], fmt='%.4f', newline=', ')
