...
env_name = "./Tac_Follow_r30_fr"  # Name of the Unity environment binary to launch
...
class PPO(object):
    def __init__(self):
        ...
        self.vs = tf.placeholder(tf.float32, [None, 2*NUM_PINS], 'state')
    ...
    def update(self):
        ...
        while not COORD.should_stop():
            if GLOBAL_EP < EP_MAX:
                ...
                s, a, r = data[:, :2*NUM_PINS], data[:, 2*NUM_PINS: 2*NUM_PINS + A_DIM], data[:, -1:]
        ...
        while not COORD.should_stop():
            ...
            for t in range(EP_LEN):
                ...
                if t > EP_LEN-1:
                    ...
                    Done = False
                try:
                    pins_x, pins_y, pins_dis_x, pins_dis_y = self.ImgProcess(img, Done)
                except:
                    print('Image Processing Error!')
