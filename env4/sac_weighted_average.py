...
def state_process(s, s0):
    ...
    processed_state=np.concatenate((s[:3], weighted_average[0])) # reduce 1 dim in weighted_average
...
# env_name="./tac_follow4_random"  # test domain transfer
env = UnityEnv(env_name, worker_id=18, use_visual=False, use_both=True)
model_path = './model/sac_weighted_average'
