...
def state_process(s, s0):
    ...
    x=s[3::3]
    z=s[5::3]
    relative_x=x-object_x
    relative_z=z-object_z

    idx_outside_pins=[0,1,2,3,4,5,6,12,13,20,21,29,30,39,40,50,51,60,61,69,70,77,78,84,85,90]
    for i in range(len(x)):
        if i not in idx_outside_pins:
            relative_dis=np.abs(relative_x-relative_x[i])+np.abs(relative_z-relative_z[i])
            closest_pins_idx=np.argsort(relative_dis)[:6]
            average_dis=np.average(relative_dis[closest_pins_idx.tolist()])
            # print(average_dis)
            if average_dis>average_dis_thre:
                x[i]=1  
            else:
                x[i]=0
        else:
    ...
    processed_state=np.concatenate((s[:3], x))
...
# choose env
env_name="./tac_follow4"
# env_name="./tac_follow4_random"

env = UnityEnv(env_name, worker_id=18, use_visual=False, use_both=True)
model_path = './model/sac_neighbor'
