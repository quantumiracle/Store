...
def state_process(s, s0):
    ...
    x=s[6::3]
    z=s[8::3]
    dis=np.abs((x-object_x)-(x0-object_x0))+np.abs((z-object_z)-(z0-object_z0))
    dis[dis<=dis_threshold]=0
    dis[dis>dis_threshold]=1
    # print(dis)

    processed_state=np.concatenate((s[:6], dis))
...
model_path = './model/td3_threshold'
