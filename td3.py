...
class TD3_Trainer():
    ...
    def save_model(self, path):
        torch.save(self.q_net1.state_dict(), path)
        torch.save(self.q_net2.state_dict(), path)
        torch.save(self.policy_net.state_dict(), path)

    def load_model(self, path):
        self.q_net1.load_state_dict(torch.load(path))
        self.q_net2.load_state_dict(torch.load(path))
...
# choose env
env_name="./tac_follow2"
env = UnityEnv(env_name, worker_id=1, use_visual=False, use_both=True)
...
for eps in range(max_episodes):
    ...
    for step in range(max_steps):
        if frame_idx > explore_steps:
            action = td3_trainer.policy_net.get_action(state, deterministic = DETERMINISTIC, explore_noise_scale=5.0)
        else:
