...
env_name = "./tac_follow3"  # Name of the Unity environment binary to launch
...
class Worker(object):
    ...
    def work(self):
        ...
        while not COORD.should_stop():
            ...
            # threshold_s=np.zeros_like(s) + np.array(s)[:6]  # keep first 6 dim, others are 0
            step+=1
            ep_r = 0
            ...
            for t in range(EP_LEN):
                ...
                # select pins with displacement larger than threshold to be 1, else 0, as observation
                dis = np.abs((pins_x-s[0]) - (self.pins_x[0]-self.object_x[0])) + np.abs((pins_z-s[2]) - (self.pins_z[0]-self.object_z[0]))
                dis_threshold=0.4
                dis[dis<=dis_threshold]=0
                dis[dis>dis_threshold]=1
                # print(dis)
                # plot_deform(np.argwhere(dis>0.5).squeeze().tolist(), 1)
                threshold_s=np.concatenate((s[:6], dis))

                a = self.ppo.choose_action(threshold_s)
                s_, r, done, info= self.env.step(a)
                # print(np.array(s_).shape)

                buffer_s.append(threshold_s)
                buffer_a.append(a)
